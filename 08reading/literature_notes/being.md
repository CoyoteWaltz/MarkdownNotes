...
2025.05.07 10:25:20 +0800

[Rest is not the opposite of work](https://www.remoteworkprep.com/blog/rest-isnt-the-opposite-of-work)

> 休息不是工作的对立面，我们常常会工作到真的撑不住了才去休息，其实我们早就应该休息，博客虽然是推荐的远程工作的休息建议，也很实用。
>
> 提出了 7 种休息方式，对应 7 个方面的活动作业。
>
> 1 物理休息，可以睡个午觉，打个小盹，切换工作场所
>
> 2 心理休息，日中冲澡，清理时刻
>
> 3 感观休息，离开电脑屏幕，做点书写，走走聊聊天
>
> 4 创意休息，出去走走感受自然，创建自己的灵感文件夹
>
> 5 情绪休息，和好友吃个饭
>
> 6 社交休息，调整日程，每天留给自己一些时间
>
> 7 灵魂休息，加入一个小组，做志愿者
>
> 当你发现你的工作仿佛是将一个拼图块强行塞入这个错误的位置，就休息一下吧，不要 burnout

2025.08.23 23:17:48 +0800

[概率思维](https://mp.weixin.qq.com/s/JQFzRPNlxoHZEskZ2aRmmQ)

> 拥有概率思维，能对日常的决策进行更高维度的思考，从而带来更新的认知迭代
>
> 其背后的精神内核是**对世界不确定性的敬畏**

2025.12.18 23:14:21 +0800

[一篇 2025 的年度总结](https://onevcat.com/2025/12/2025-final/)

> 把深处洪流时代的我的想法清晰的表达了出来，很是感叹这一年
>
> 摘录一段评论区醍醐灌顶的内容：
>
> > 现在大家讨论 AI 编程可以多大程度提高生产力的情况，感觉和当年提出「没有银弹」时的背景有点像，40 多年前对软件危机进行反思的时候，其实已经讨论过类似的情况：即使存在「具有完美智能的编程机器」，也无法在 10 年内带来数量级的生产力提升，因为软件的本质复杂度（Essential Complexity）是无法通过技术手段消除的。
> >
> > AI 编程的第一性原理，我觉得可以从信息论的角度：即**描述一个软件所需的信息量是存在下限的**，描述越简单，产出越容易偏离需求，描述越冗余，越能实现对需求的控制，不存在一种描述，可以既省力又能准确反映复杂的需求。
> >
> > 根据描述产出复杂的软件代码的过程，本质是一个对信息（需求）进行「解压缩」的过程，古法编程是靠程序员的知识和经验优化这个「解压缩」过程，现在大模型之所以也能实现这种从需求到代码的转换，是因为模型预训练阶段对整个互联网上的工程实践和代码模式进行了「有损压缩」，而 Transformer 架构引导了这个巨大的「压缩包」如何高质量「解压」出符合 Prompt 的工程代码。
> >
> > 如果基于这个原理进行推论，把编程类比为对需求的「解压缩」，分析这个压缩算法：
> >
> > | 维度     | 涵义                 | 程序员                         | 大模型                     |
> > | -------- | -------------------- | ------------------------------ | -------------------------- |
> > | 压缩率   | 描述软件所需的信息量 | 准确实现抽象需求的能力         | Prompt ➗ 有效输出         |
> > | 字典     | 提高压缩率的信息结构 | 大脑积累的知识和技能、项目知识 | 预训练参数、上下文         |
> > | 解压速度 | 产出代码的速度       | 编码效率、工具·框架熟练度      | 推理效率                   |
> > | 压缩质量 | 产出的代码质量       | 架构设计、Debug 能力           | Fine-tuning 效果、工具调用 |
> >
> > 上面的类比不算恰当，但是还是可以解释一些趋势和现象：
> >
> > - 大模型已经在「字典」「解压速度」上完成了对人类的碾压，此刻正在「压缩质量」上追赶顶级水平
> > - 程序员在 Vibe Coding 时的不安和钝感是因为包含项目细节的「字典」的绝大部分已经不在自己脑中
> > - 不同程序员对 Vibe Coding 的评价两极分化，（不考虑 skill issue）是因为「压缩率」和「压缩质量」与「被压数据」（工作内容）高度相关：常见、重复的工作很容易被「压缩」，而对前沿、复杂的工作的「压缩率」则可能超过 100%，越压越大
> >
> > 抛开创作者的身份，从程序员角度，我个人在大模型时代的反思是：
> >
> > - 开创性工作上人类是不可能被 LLM 取代的，至少在新一轮算法架构突破之前（也许是世界模型，也许是别的，中间很可能会经历一轮新的 [AI 寒冬](https://zh.wikipedia.org/wiki/人工智慧低谷)）
> > - 如果你在自己的核心工作上可以感受到 Vibe Coding 带来的巨大提升，这不代表你跟上了时代，相反，这可能是一个危险信号，说明你的工作内容缺乏深度，只是在不断执行明确的需求，几年内会被聪明的 PM 替代，他们是需求层面的专家
> > - 长时间依赖 AI 会让自己习惯在一个更高的抽象层次上思考，丧失部分对底层的敏感度和掌控能力，这对一些工作来说是致命的（性能优化、安全性、库开发等），而底层工作是无法被替代的，哪怕是人类自己的智能，也要建立在每个被正确维护的细胞之上
> >
> > 总结一下就是多做 [Deep Work](https://youtu.be/7uUl_aTbOzQ?si=JxTgAval2WVuyiKV) ，多做那些为行业开疆拓土的工作，和那些开始时无从下手、结束时能收获全新认知的工作，这样也许还能苟活到 AGI 降临的那天。
> >
> > （醒太早，借你的评论区输出一下自己的思考，长文失礼）

2026.01.11 15:09:47 +0800

[how i reboot my social life](https://takes.jamesomalley.co.uk/p/this-might-be-oversharing)
...
>
> - 社区会形成隐形规范和压力（“规范之笼”）。
> - 容易压制异见、鼓励从众，妨碍独立思考和求真
>
> **人类依然需要群体连接**，即便这会让“绝对理性”更复杂。
>
> 很多人的社交问题并非个人失败，而是**结构性环境变化**（疫情、远程办公、平台瓦解）。
>
> 不要等“合适的理由”：
>
> > **直接邀请人来做点事**
